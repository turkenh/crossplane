...
go-build:
  ...
  ARG GOOS=${TARGETOS}
  LET INTERNAL_CROSSPLANE_VERSION=$(echo ${CROSSPLANE_VERSION}| sed 's/[\.,-]up.*//')
  ARG GOFLAGS="-ldflags=-X=github.com/crossplane/crossplane/internal/version.version=${INTERNAL_CROSSPLANE_VERSION}"
  ARG CGO_ENABLED=0
  FROM +go-modules
  LET ext = ""
  IF [ "$GOOS" = "windows" ]
